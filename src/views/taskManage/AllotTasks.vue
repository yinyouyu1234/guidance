    ...
    <search>
      <el-form :inline="true">
        <el-form-item label="巡检人员">
          <el-input size="mini" v-model="condition.user_name" placeholder="请输入巡检人员"></el-input>
        </el-form-item>
        <el-form-item label="巡检路线">
          <el-input size="mini" v-model="condition.patrol_route_name" placeholder="请输入巡检路线"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    over(row) {
      this.$axios
        .get(`${this.api}/flightstask/endTask?task_id=${row.id}`)
        .then(res => {
            ...
            data.forEach((item, i) => {
              ...
              item.buttonInfo = [
                ...
                },
                {
                  name: "over",
                  disabled:
                    item.flights_type == 1 && item.task_status == 1
                      ? false
                      : true,
                  type: "danger",
                  label: "结束"
